...
def _calculate_receita_escritorio(user_id, clientes):
    """Calcula receita do escritório usando lógica similar ao Dashboard"""
    ...
    try:
        # Receita ativa via alocações efetivadas
        ...
        for alocacao in alocacoes:
            # Verificar se a alocação é do mês atual
        ...
        # Receita recorrente (passiva) - usando lógica do Dashboard
        receita_recorrente = _calculate_receita_escritorio_recorrente(user_id, clientes)

        # Bônus ativo (simplificado para o admin)
        ...
        current_app.logger.error(f"Erro calculando receita escritório para user_id {user_id}: {e}")
        return 0.0

def _calculate_receita_assessor_recorrente(user_id):
    """Calcula receita assessor recorrente usando EXATA lógica do Dashboard"""
    if not supabase or not user_id:
        return 0.0
    ...
    try:
        # 1. Buscar último mês disponível (mesma lógica do Dashboard)
        ...
        # 2. Buscar preferências do usuário (mesma lógica do Dashboard)
        ...
        # 3. Buscar receitas do último mês (mesma lógica do Dashboard)
...
def _calculate_receita_escritorio_recorrente(user_id, clientes):
    """Calcula receita escritório recorrente baseada na receita assessor recorrente"""
...
def _calculate_bonus_ativo(user_id, mes_atual):
    """Calcula bônus ativo do mês usando EXATA lógica do Dashboard"""
    if not supabase or not user_id:
        return 0.0

    def _calcular_valor_liquido_bonus(valor_bonus, liquido_assessor):
        """Mesma função do Dashboard para calcular valor líquido"""
        if liquido_assessor:
            return valor_bonus
        else:
    ...
    try:
        ...
        # Tentar com colunas novas primeiro (mesma lógica do Dashboard)
        try:
...
def _calculate_receita_assessor(receita_escritorio, clientes, user_id):
    """Calcula receita do assessor usando lógica do Dashboard - INCLUI BÔNUS"""
